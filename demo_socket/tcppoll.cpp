...
int bb=0;
  while(true)
  {
    ...
    fd_set tmpfds = readfds;    // 读fds
    fd_set tmpfds1 = readfds;   // 写fds
    /**
    ...
    /* 
     * 阻塞监听事件，有事件相应select()函数就返回，置有事件发生的socket为1，其他为空
     * ifds：事件变化的个数或者监视结果
     * 超时时间暂时不设置
    */
    int ifds = select(maxfd+1, &tmpfds, /*&tmpfds1*/ NULL, NULL, NULL);
    if (ifds < 0)    // 函数调用失败
    {
