...
public:
  ...
  bool SplitBuffer(char *strBuffer);                       // 把获取到的文件内容写入到结构体中
...
bool CZHOBTMIND::InsertTable()
{
  ...
  if (m_stmt.m_state == 0)
  {
    m_stmt.connect(&conn);  // stmt需要配置连接
  ...
  if (m_stmt.execute() != 0)
  {
    ...
    {
      logfile.Write("m_stmt.execute() failed.\n%s\n%d：%s\n", m_stmt.m_sql, m_stmt.m_cda.rc, m_stmt.m_cda.message);
      logfile.Write("行数据：%s 有误。\n", m_buffer);
    }
    return false;
  }
  return true;
...
bool _obtmindtodb(const char *datapath, char *connstr, char *charset)
{
  ...
  while (true)
  {
    ...
    {
      // 将每一行的内容解析到结构体中
      if (file.FFGETS(strBuffer, 1023, "<endl/>") == false) break;  // 会清空buffer的

      // 1
      if (zhobtmind.SplitBuffer(strBuffer) == false) continue;
